<!doctype html>

<html>
  <head>
    <title>Introduction to Law</title>
    <meta charset="utf-8">

    <link rel="stylesheet" href="./css/stylesheet.css" type="text/css" />
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/font-awesome/4.4.0/css/font-awesome.min.css">
    <meta name="viewport" content="width=1100; initial-scale=1.0; target-densityDpi=device-dpi" />

    <meta name="keywords" content="Canadian, Law, Introduction, Overview, Summary, Interconnected" />
    <meta name="description" content="An introduction to Canadian Law." />
    <meta name="author" content="Isaac McQuaide" />
    <meta name="robots" content="all" />

    <link rel="shortcut icon" href="img/favicon.ico" />

    <script src="https://ajax.googleapis.com/ajax/libs/jquery/1.11.2/jquery.min.js"></script>

    <script src="../javascript/universal.js"></script>
  </head>

  <body>
    <?php
      include 'php/header.php';
    ?>
    <div class="title-section">
      <h1 class="title">Overview of Law</h1>
      <p class="title-description">Looking at law from a conceptual perspective</p>
    </div>
    
    <div class="section section-white">
      <div class="links links-small">
        <div class="link">
          <a href="natureoflaw.php">The Nature of Law</a>
          <p class="link-description">Lorem ipsum...</p>
        </div>
        <div class="link">
          <a href="introtolaw.php">Introduction to Law</a>
          <p class="link-description">Lorem ipsum...</p>
        </div>
      </div>
    </div>
    <?php
      include 'php/footer.php';
    ?>
  </body>
</html>
